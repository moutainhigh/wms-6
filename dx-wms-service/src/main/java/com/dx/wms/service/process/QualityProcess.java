...
@Service
public class QualityProcess extends CustomerRoleProcess {
    ...
    public String init(ParamProcess param) {
        return "checkProcess/qualityCheck";
    }

    @Override
    public Boolean supports(ParamProcess param) {
        return Assert.equals(param.getBiz(), ApplyBiz.QUALITY);
    }
    ...
    public PaginationResult<List<ResultProcess>> query(ParamProcess param, Pagination page) {
        ...
        List<String> proIns = workFlowService.getProIns(userId, ProcessWealthConstans.TASK_SALESERVICEQUALITY);
        if (Assert.checkParam(proIns)) {
            param.setProIns(proIns);
        }
        return dalClient.queryForList("salesCustomer.queryForPage", MapUtils.obj2Map(param), ResultProcess.class, page);
    }
    ...
    public String createInit(ParamProcess param) {
        return "checkProcess/qualityCheckDetail";
    }
