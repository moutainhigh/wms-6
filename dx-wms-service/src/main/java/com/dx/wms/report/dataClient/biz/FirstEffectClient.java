...
@Service
public class FirstEffectClient<P, R> extends DataClientObserver<P, R>{
	

	@Override
	public Boolean supports(ReportParamDto<P, R> param) {
		return ReportType.EFFECTFIRST.getCode().equals(param.getReportType())  ;
	}
	...
	protected void execute(ReportParamDto<P, R> param) {
		...
		param.setResult((List<R>) getFirstView(param,Boolean.TRUE));
	}
