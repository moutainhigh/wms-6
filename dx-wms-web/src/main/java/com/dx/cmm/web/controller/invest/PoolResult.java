...
public class PoolResult extends Result {
    ...
    public PoolResult(InvestResult invest, ILenderApplyService lenderApplyService, IAMService amService,
        ...
        if (Assert.checkParam(apply)) {
            ...
            setTeamView(Assert.checkParam(amService.queryOrgById(getTeamId())) ? amService.queryOrgById(getTeamId()).getName() : "-");
            ...
            setLenderDateView(Assert.checkParam(getBizDateBegin(),getBizDateEnd()) ?MessageFormat.format("{0} 至  {1}", DateUtils.formatForDay(getBizDateBegin()),
            DateUtils.formatForDay(getBizDateEnd())) : "-");
            setApplyDateView(Assert.checkParam(getApplyDate()) ? DateUtils.formatForDay(getApplyDate()) : "-");
