...
<div class="body" >
    ...
    <div>
        ...
        <p class="txt10 font_h txt_indent" style="margin-top: -0.4%">在您接受该批债权转让并按时支付对价的情况下，预期您的出借获益情况如下：<span style="float: right;">货币单位：人民币</span></p>
    </div>
    <div style="margin-top: 5%">
        <table style="font-size:12px;width: 100%;text-align: center;border: 0.6px solid black;border-collapse: collapse;border-spacing: 0;">
            <tr style="page-break-inside: avoid;">
                ...
                <td style="width: 50px;border: 0.6px solid black;padding:18px 0px;">资金出借及<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>回收方式</td>
                <td style="width: 95px;border: 0.6px solid black;padding:18px 0px;">初始出借<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>日期</td>
                <td style="width: 100px;border: 0.6px solid black;padding:18px 0px;">初始出借<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>金额</td>
                <td style="width: 100px;border: 0.6px solid black;padding:18px 0px;">下一个<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>报告日</td>
                <td style="width: 90px;border: 0.6px solid black;padding:18px 0px;">下一个报告期借款人<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>应还款额</td>
                <td style="width: 50px;border: 0.6px solid black;padding:18px 0px;">服务费</td>
                <td style="width: 100px;border: 0.6px solid black;padding:18px 0px;">预计下一个报告日<#if report.isView?exists && report.isView!=""></br><#else><br></br></#if>您的资产总额</td>
            </tr>
            <tr style="page-break-inside: avoid;line-height: 24px;">
                ...
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.initMatchTimeView}</td>
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.initTotalAmountView}</td>
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.initBillDateView}</td>
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.repayView}</td>
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.accountAmountView}</td>
                <td style="border: 0.6px solid black;padding:2px 0px;">${report.nextTotalAmountView}</td>
            </tr>
        </table>
    </div>
    <div>
    ...
    <div style="page-break-inside: avoid">
	    <div>
	        <h4 style="text-align: center;margin-top: 10px" class="font_h">转让人声明</h4>
	         <p class="txt9 font_h">
	            本人自愿将上述债权转让给受让人。如果受让人对上述债权转让没有任何异议，须于<span style="border-bottom:0.6px solid black;">${report.initMatchTimeView}</span>前将上述对价共计：
	        </p>
	        <p class="txt9 font_h">人民币（大写）<span style="border-bottom:0.6px solid black;">${report.initTotalAmountChsView}</span>（人民币小写￥<span style="border-bottom:0.6px solid black;">${report.initTotalAmountView}</span>）支付到如下指定账户。</p>
	    </div>
    </div>
    <!-- page-break-inside: avoid 保证分页不被分割,确保完整性 -->
   	<div style="line-height: 15px; margin-left: 40%; margin-top: 5%; margin-bottom: 5%;page-break-inside: avoid;" class="txt_bold" >
    ...
    <div style=" margin-left: 20%;page-break-inside: avoid;">
        ...
        <p style="font-size: 12px; line-height: 30px;">日期：${report.initMatchTimeView}<span style="margin-left: 21.5%">日期：${report.initMatchTimeView}</span></p>
    </div>
