...
var isOldPayInfo = false;
var isOldRefundInfo = false;
$(function(){
	//页面初始化时，使div的滚动条显示在最顶端
	$(".tab-content").scrollTop(0);
	//当出借方式为达信宝&月利盈时,可进行变更且只能为达信宝&月利盈
	if($("#productId").val()==11||$("#productId").val()==12){
		var productId = $("#productId").val();
		$("#productId").empty().
		append("<option value='11'>达信宝</option>").
		append("<option value='12'>月利盈</option> ");
		$("#productId").val(productId); 
	}
	//当省市不存在时,支行为其他,其他内容为支行内容
	$("#payOtherAddress").closest(".control-group").hide();
	$("#refundOtherAddress").closest(".control-group").hide();
	if($("#payprovinceRegionCode").val()==-1 && $("#paycityRegionCode").val()==-1){
		isOldPayInfo = true;
		$("#paySubBank option").remove();
		$("#paySubBank").append("<option value='-1'>请选择支行</option>");
		$("#paySubBank").append("<option selected='selected' >"+$("#payOtherAddress").val()+"</option>");
		$("#paySubBank").append("<option value='-2'>其他</option>");
	}
	if($("#refundprovinceRegionCode").val()==-1 && $("#refundcityRegionCode").val()==-1){
		isOldRefundInfo = true;
		$("#refundSubBank option").remove();
		$("#refundSubBank").append("<option value='-1'>请选择支行</option>");
		$("#refundSubBank").append("<option selected='selected'>"+$("#refundOtherAddress").val()+"</option>");
		$("#refundSubBank").append("<option value='-2'>其他</option>");
	}
});
...
function save(tab, navigation, index) {
	...
	if (index == 2) {
		...
		if($("#payprovinceRegionCode").val()==-1 || $("#paycityRegionCode").val()==-1
			|| $("#refundprovinceRegionCode").val()==-1 || $("#refundcityRegionCode").val()==-1){
			$.dopAlert("请选择银行所属省市");
			return false;
		}
		...
		if (flag) {
			$
					.ajax({
						url : base + '/operate/save.json',// 请求url
						type : "POST",
						async : false,
						dataType : "json",
						contentType : "application/json",
...
function getLenderApply() {
	...
	lenderApply.lenderCode=$("#lenderCode").val();
	return lenderApply;
}
...
function getRefundCustFinance() {
	...
	if($("#refundSubBank").val()==-2){
		custFinance.subBank = $("#refundOtherAddress").val();
	}else{
...
function getPayCustFinance() {
	...
	if($("#paySubBank").val()==-2){
		custFinance.subBank = $("#payOtherAddress").val();
	}else{
...
$("#payWayId").change(function() {
	...
	//重置
	$("#signMobile").val("");
	$(this).closest(".span6").siblings(".span6").removeClass("success").removeClass("error").find(".validate-inline").text("");
});
...
$("#refundprovinceRegionCode").change(function() {
	$("#refundOtherAddress").closest(".control-group").hide();
	$("#refundcityRegionCode option").remove();
	$("#refundcityRegionCode").append("<option value='-1'>请选择市</option>");
	$("#refundBankCategory").val("-1");
	$("#refundSubBank").val("-1");
	$.getJSON(base + '/common/region.json?param=' + $("#refundprovinceRegionCode").val(), function(data) {
		$.each(data, function(index, item) {
			$("#refundcityRegionCode").append("<option value='" + item.area_code_id + "'>" + item.area_code_name + "</option>");
		});
	});
});
$("#payprovinceRegionCode").change(function() {
	$("#payOtherAddress").closest(".control-group").hide();
	$("#paycityRegionCode option").remove();
	$("#paycityRegionCode").append("<option value='-1'>请选择市</option>");
	$("#payBankCategory").val("-1");
	$("#paySubBank").val("-1");
	$.getJSON(base + '/common/region.json?param=' + $("#payprovinceRegionCode").val(), function(data) {
		$.each(data, function(index, item) {
			$("#paycityRegionCode").append("<option value='" + item.area_code_id + "'>" + item.area_code_name + "</option>");
		});
	});
});
...
$("#payBankCategory").change(function() {
	$("#paySubBank option").remove();
	$("#paySubBank").append("<option value='-1'>请选择支行</option>");
	$("#payOtherAddress").closest(".control-group").hide();
	...
	$.ajax({
		url :base + '/process/subBank.json?bankCode=' + $("#payBankCategory").val()+'&paycityRegionCode='+$("#paycityRegionCode").val()+
		'&payprovinceRegionCode='+$("#payprovinceRegionCode").val(),// 请求url
		type : "POST",
		async : false,
		dataType : "json",
		contentType : "application/json",
		...
		success : function(data) {
			if(data!=null&&data!=""){
				$.each(data, function(index, item) {
					$("#paySubBank").append("<option value='" + item.subBankName + "'>" + item.subBankName + "</option>");
				});
			}
				$("#paySubBank").append("<option value='-2'>其他</option>");
		},
...
$("#refundBankCategory").change(function() {
	$("#refundSubBank option").remove();
	$("#refundSubBank").append("<option value='-1'>请选择支行</option>");
	$("#refundOtherAddress").closest(".control-group").hide();
	...
	$.ajax({
		url :base + '/process/subBank.json?bankCode=' + $("#refundBankCategory").val()+'&paycityRegionCode='+$("#refundcityRegionCode").val()+
		'&payprovinceRegionCode='+$("#refundprovinceRegionCode").val(),// 请求url
		type : "POST",
		async : false,
		dataType : "json",
		contentType : "application/json",
		...
		success : function(data) {
			if(data!=null&&data!=""){
				$.each(data, function(index, item) {
					$("#refundSubBank").append("<option value='" + item.subBankName + "'>" + item.subBankName + "</option>");
				});
			}
				$("#refundSubBank").append("<option value='-2'>其他</option>");
		},
...
$("#paySubBank").change(function(){
	if($("#paySubBank").val()==-2){
		...
		$("#payOtherAddress").closest(".control-group").show();;
...
$("#refundSubBank").change(function(){
	if($("#refundSubBank").val()==-2){
		if(!isOldRefundInfo){
			$("#refundOtherAddress").val("");
		}
		$("#refundOtherAddress").closest(".control-group").show();;
...
//			}
//			if ($("#contractCode").val() != ''
//					&& $("#contractCode").val() != varContractCode && reg.test($("#contractCode").val()))  {
//				var custInfoCheckVo = {};
//				custInfoCheckVo.contractCode = $("#contractCode").val();
//				$.ajax({
//					url : base + '/common/check_contract_code.json',// 请求url
