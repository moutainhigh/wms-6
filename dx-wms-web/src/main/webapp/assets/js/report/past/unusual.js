			...
			.dataTable(
					{
						...
						"sScrollX" : "100%",
						"sScrollXInner" : "110%",
						"sAjaxSource" : 'list_plus.json',// 请求url
						"sServerMethod" : "POST",
...
//								|| $( e.target ).closest(".dataTables_info").length != 0
	...
	function execute(biz , pastParamVo){
		$.ajax({
			...
			url : base+ "/report/past/"+biz+"_report_plus.json",
	...
	$(".review").live("click",function() {
		...
		$.ajax({
				...
				dataType : "html",
				contentType : "application/json",
				url : base+ "/report/past/preview_plus.json",// 请求url
	...
	function dealProtocolFileName(tds, processResult,
		...
		// 获取匹配日期
		var matchDatePlus = $(tds[26]).text().replace("-", "")
				.replace("-", "");
		var name = $(tds[2]).text()+" ";
		// 拼接获取协议文件名称
		var protocolFileName = suffix + matchDatePlus + " "+name+lenderCode;
