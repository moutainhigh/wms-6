...
//	initPieCharts();
//	initColumnChart();
	$("#pendingQualityApplyNum").click(function(){
		window.location.href = base + "/process/quality.htm";
	});
	
	$("#refusedApplyNum").click(function(){
		window.location.href = base + "/wealthManagementInfo/list.htm";
	});
...
//	series = series < 3 ? 3 : series;
	
	var checkResArr = checkRes.split(";");
	if(null == checkResArr || checkResArr.length == 0) {
		return ;
	}
	var res = new Array();
...
function showTooltip(x, y, contents) {
    $('<div id="tooltip" class="chart-tooltip"><div class="date">' + getLenderProportionInfo(contents,2) + '<\/div><div class="label label-success">Amount(￥): ' + getLenderProportionInfo(contents,3) + '<\/div><\/div>').css({
        position: 'absolute',
        display: 'none',
        top: y - 100,
        width: 75,
...
//	if ($('#site_activities_big').size() != 0) {
//	    var previousPoint2 = null;
//	    $('#site_activities_loading_big').hide();
//	    $('#site_activities_content_big').show();
//
//	    var activities = [
//	        [1, 10],
//	        [2, 9],
...
//	    });
//
//	    $("#site_activities_big").bind("plothover", function (event, pos, item) {
...
//	    });
//
//	    $('#site_activities_big').bind("mouseleave", function () {
//	        $("#tooltip").remove();
//	    });
//	    
//	}
